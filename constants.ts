...
<div class="space-y-3 text-sm">
    <p>To add new words by uploading a file, you need a CSV (Comma-Separated Values) file with a specific format. It must contain **42 columns** (or 43 with the optional ID column).</p>
    <p><strong>1. Header Row:</strong> The very first line of your file MUST be the header row, exactly like this (all one line):</p>
    <pre class="bg-base-300 p-2 rounded-md text-xs overflow-x-auto whitespace-pre-wrap word-break"><code>Source,Subtopic1,Subtopic2,WordType,Swedish,SwedishExample,en_Word,en_Example,es_Word,es_Example,fa_Word,fa_Example,el_Word,el_Example,uk_Word,uk_Example,ru_Word,ru_Example,hi_Word,hi_Example,bn_Word,bn_Example,sq_Word,sq_Example,tr_Word,tr_Example,ms_Word,ms_Example,fil_Word,fil_Example,Present,PresentTranslation,PresentExample,PresentExampleTranslation,Preteritum,PreteritumTranslation,PreteritumExample,PreteritumExampleTranslation,Supinium,SupiniumTranslation,SupiniumExample,SupiniumExampleTranslation,ID</code></pre>
    <p><strong>2. Data Rows:</strong> Each subsequent line represents one Swedish word and all its translations. Fields are separated by commas. Examples are optional. The ID field can be left blank.</p>
    <p><strong>Example Row:</strong></p>
    <pre class="bg-base-300 p-2 rounded-md text-xs overflow-x-auto whitespace-pre-wrap word-break"><code>Rivstart A1,Kapitel 1,Verbs,verb,att arbeta,"Jag arbetar...","to work","I work...","trabajar","Yo trabajo...",,,,,,,,,,,,,,,,,,,arbetar,"works","Han arbetar idag.","He works today.",arbetade,"worked","Vi arbetade igår.","We worked yesterday.",arbetat,"worked","De har arbetat.","They have worked."</code></pre>
...
<div class="space-y-3 text-sm">
    <p>Para añadir nuevas palabras subiendo un archivo, necesitas un archivo CSV (Valores Separados por Comas) con un formato específico. Debe contener **42 columnas** (o 43 con la columna ID opcional).</p>
    ...
    <pre class="bg-base-300 p-2 rounded-md text-xs overflow-x-auto whitespace-pre-wrap word-break"><code>Source,Subtopic1,Subtopic2,WordType,Swedish,SwedishExample,en_Word,en_Example,es_Word,es_Example,fa_Word,fa_Example,el_Word,el_Example,uk_Word,uk_Example,ru_Word,ru_Example,hi_Word,hi_Example,bn_Word,bn_Example,sq_Word,sq_Example,tr_Word,tr_Example,ms_Word,ms_Example,fil_Word,fil_Example,Present,PresentTranslation,PresentExample,PresentExampleTranslation,Preteritum,PreteritumTranslation,PreteritumExample,PreteritumExampleTranslation,Supinium,SupiniumTranslation,SupiniumExample,SupiniumExampleTranslation,ID</code></pre>
    <p><strong>2. Filas de datos:</strong> Cada línea siguiente representa una palabra en sueco y todas sus traducciones. Los campos están separados por comas. Los ejemplos son opcionales. El campo ID puede dejarse en blanco.</p>
