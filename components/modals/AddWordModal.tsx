...
const AddWordModal: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!source || !subtopic1 || !subtopic2 || !swedish || !sourceWord) {
            alert(t('modal.addWord.fillRequired'));
            return;
        }
    ...
    const renderSuggestButton = (type: 'swedish' | 'source') => (
      <button type="button" onClick={() => handleSuggestExample(type)} disabled={isSuggesting !== null} className="text-xs text-blue-400 hover:underline disabled:opacity-50">
        {isSuggesting === type ? t('modal.addWord.aiThinking') : t('modal.addWord.suggestExample')}
      </button>
    );
    ...
    return (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-base-200 p-6 rounded-lg shadow-xl z-50 w-11/12 max-w-lg">
            <h3 className="text-lg font-bold mb-4">{wordToEdit ? t('modal.addWord.editTitle') : t('modal.addWord.addTitle')}</h3>
            <form onSubmit={handleSubmit}>
                <div className="space-y-3">
                    <input type="text" name="source" value={formData.source} onChange={handleChange} placeholder={t('modal.addWord.source')} required className="w-full p-2 bg-base-300 rounded-md" />
                    <input type="text" name="subtopic1" value={formData.subtopic1} onChange={handleChange} placeholder={t('modal.addWord.subtopic1')} required className="w-full p-2 bg-base-300 rounded-md" />
                    <input type="text" name="subtopic2" value={formData.subtopic2} onChange={handleChange} placeholder={t('modal.addWord.subtopic2')} required className="w-full p-2 bg-base-300 rounded-md" />
                    <hr className="border-base-300 my-3" />
                    <input type="text" name="swedish" value={formData.swedish} onChange={handleChange} placeholder={t('modal.addWord.swedish')} required className="w-full p-2 bg-base-300 rounded-md" />
                    <div className="relative">
                      <input type="text" name="swedishExample" value={formData.swedishExample} onChange={handleChange} placeholder={t('modal.addWord.swedishExample')} className="w-full p-2 bg-base-300 rounded-md" />
                      <div className="absolute right-2 top-1/2 -translate-y-1/2">{renderSuggestButton('swedish')}</div>
                    </div>
                    <hr className="border-base-300 my-3" />
                    <label className="text-sm font-medium">{t('modal.addWord.translation')}<span className="font-bold">{currentLanguageInfo.englishName}</span></label>
                    <input type="text" name="sourceWord" value={formData.sourceWord} onChange={handleChange} placeholder={t('modal.addWord.wordIn', { language: currentLanguageInfo.englishName })} required className="w-full p-2 bg-base-300 rounded-md" />
                     <div className="relative">
                        <input type="text" name="sourceWordExample" value={formData.sourceWordExample} onChange={handleChange} placeholder={t('modal.addWord.exampleIn', { language: currentLanguageInfo.englishName })} className="w-full p-2 bg-base-300 rounded-md" />
                ...
                <div className="flex justify-end gap-3 mt-5">
                    ...
                    <button type="submit" className="px-4 py-2 bg-accent text-accent-content rounded-md hover:bg-accent-focus">{t('action.saveWord')}</button>
                </div>
            </form>
        </div>
    );
